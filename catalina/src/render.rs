...
impl Render {
    ...
    pub fn render_encoding_coarse(
        ...
        use catalina_encoding::RenderConfig;
        ...
        let scene_buf = ResourceProxy::Buffer(recording.upload("catalina.scene", packed));
        let config_buf = ResourceProxy::Buffer(
            recording.upload_uniform("catalina.config", bytemuck::bytes_of(&cpu_config.gpu)),
        );
        ...
        match fine.aa_config {
            ...
            _ => {
                if self.mask_buf.is_none() {
                    ...
                    let buf = recording.upload("catalina.mask_lut", mask_lut);
